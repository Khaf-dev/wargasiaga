...
# Forward declaration untuk type hinting (hindari circular import)
if TYPE_CHECKING:
...
class IncidentResponse(Base, UUIDMixin, TimestampMixin):
    ...
    # Gunakan native_enum=False untuk membuat CHECK constraint, 
    # bukan tipe ENUM native di DB.
    response_type: Mapped[ResponseType] = mapped_column(
        SAEnum(ResponseType, name="response_type_check", native_enum=False, length=20)
    )
    ...
    # ─── Phase 4.2 relationships (back_populates dari Incident & User) ──
